...
\item[] POST -- Submit an edit of this content piece.
\item[] \url{/{id}}
\begin{enumerate}
\item[] GET -- Retrieve the edit of this content piece with edit\_id \{id\}
\item[] GET \url{?check_conflict={bool}} -- Retrieve the edit of this content piece \par with edit\_id \{id\} and, if its a validating edit and \{bool\} is true, \par include a boolean conflict indicator (indicating whether this edit \par likely conflicts with other validating edits). % Since checking for conflicts could be a time-intensive operation (e.g. taking many tens or hundreds of milliseconds)
\item[] \url{/votes}
\begin{enumerate}
...
\item[] DELETE -- Delete a user account.	% Requires resubmitting login info: email + password
\item[] \url{/sessions}
\begin{enumerate}
\item[] POST -- Create a new session. 	% i.e. login
\item[] DELETE -- Delete a session.	% i.e. logout
\end{enumerate}
\item[] \url{/{id}}
\begin{enumerate}
...
\item[] \url{/reports}
\begin{enumerate}
%\item[] GET -- Get a list of all reports, sorted in descending chronological order by \par creation timestamp and paginated with 10 results per page.
\item[] GET \url{?{id_type}={id}&report_status={status}} -- Retrieve a list of all user \par reports with \{id\_type\} (user\_id, admin\_id, content\_id, or ip\_address) \par matching \{id\} and report status matching \{status\} (open or resolved), \par paginated with 10 items per page and sorted in ascending chronological \par order by creation timestamp or, if present, resolution timestamp.
