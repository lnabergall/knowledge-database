...
\item[] GET -- Retrieve a list of validating edits (sorted in descending order by \par creation date) and a list of closed edits (accepted and rejected, \par sorted in descending order by validation date), both paginated with \par 10 results per page. Also include the a count of the number of \par validating edits and the number of closed edits
\item[] \url{/{id}}
\begin{enumerate}
...
\item[] \url{/users}
\begin{enumerate}
\item[] POST -- Registers a new user
